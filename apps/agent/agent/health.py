...
from agent.providers.settings import provider_settings_available

log = create_logger("agent.health")


DEFAULT_HEALTH_PORT = 8082
"""Default agent health port for local/dev.

8081 is already used by Appwrite Realtime in the local Docker stack, so the
agent must not default there or startup never reaches the LiveKit worker
registration path.
"""
...
def _check_providers_configured() -> bool:
    """Provider credentials are present for the active realtime mode.

    Default cascade mode requires the Qwen-VL/Qwen credentials accepted by
    `provider_settings_available()`; Gemini Live mode requires the Gemini key.
    """
    return provider_settings_available(os.environ)
...
def resolve_health_port(env: dict[str, str] | None = None) -> int:
    """Resolve the configured health port.

    Falls back to a local-safe default that does not collide with the Docker
    stack's Appwrite Realtime port.
    """
    source = env if env is not None else os.environ
    return int(source.get("HEALTH_PORT", str(DEFAULT_HEALTH_PORT)))
...
def start_health_server(port: int | None = None) -> ThreadingHTTPServer:
    ...
    Port: `HEALTH_PORT` env (default 8082). Binds 0.0.0.0 so k8s probes can
    reach it across the pod network.
    """
    resolved_port = port if port is not None else resolve_health_port()
