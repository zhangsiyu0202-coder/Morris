...
def main() -> None:
    ...
    # The local dev host may export an HTTP proxy that breaks the worker's
    # websocket registration path (`/agent`) with a proxy-layer 502.
    cli.run_app(WorkerOptions(entrypoint_fnc=entrypoint, prewarm_fnc=prewarm, http_proxy=None))
