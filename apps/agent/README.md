...
- **runtimeStudy/workflowConfig + provider creds present** → full voice
  interview: an `AgentSession` (Qwen ASR/TTS + Qwen-VL LLM) driven by the
  ...
  frontend, uploads the finalized recording directly, and dispatches
  `finalizeInterviewSession` for finalized transcript/session/usage persistence.
...
| `agent/providers/` | Qwen-VL (LLM) + Qwen (ASR/TTS) adapters; Gemini Live optional | settings pure, factories lazy |
...
> It is **not** used against OpenAI: it is pointed at Qwen-VL (LLM) and
...
| `QWEN_API_KEY` / `DASHSCOPE_API_KEY` | Qwen-VL + Qwen speech key (required in cascade mode) |
| `QWEN_BASE_URL` / `QWEN_VL_MODEL` / `QWEN_ASR_MODEL` / `QWEN_TTS_MODEL` / `QWEN_TTS_VOICE` | Override Qwen endpoint/models (optional) |
...
| `GEMINI_API_KEY` / `GOOGLE_API_KEY` | Gemini Live key (required only when `MERISM_GEMINI_LIVE=1`) |

If the active mode's provider credentials are absent (cascade: Qwen-VL + Qwen
speech; Gemini Live: Gemini key), the worker falls back to the UI-only RPC
bridge. If Appwrite creds are absent, the interview still runs but results are
not persisted.
