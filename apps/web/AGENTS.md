...
| `app/page.tsx` | Server redirect to `/home` (root URL just forwards into the researcher workspace). Unauthenticated visitors are intercepted by `middleware.ts` and sent to `/login?callbackUrl=/`. |
...
| `lib/hooks/` | React hooks (`use-*` 命名). `use-live-interview.ts` (LiveKit React SDK reading `merism.interviewState`), `use-interview-bootstrap.ts` (token + device pre-flight). |
...
| `lib/mock/` | **Workspace mock fixtures** (`workspace.ts`, `workspace-billing.ts`) backing the v0-generated editor + billing settings draft. Slated for removal alongside `survey-editor` / `workspaces-billing` sub-specs. Do NOT extend. |
...
| `lib/mock/workspace.ts` + `lib/mock/workspace-billing.ts` | v0 editor + billing 设置 draft 的 mock fixture. 等 `survey-editor` / `workspaces-billing` sub-spec 接真数据替换. |
