...
export function BillingSettings({
  ...
  return (
    <div className="mx-auto max-w-3xl px-8 py-10">
      ...
      {portalState ? (
        <div className="mt-4 rounded-lg border border-ink-200 bg-ink-0 px-4 py-3 font-reading text-body-sm text-ink-600 shadow-sm">
          {portalState === "unavailable"
            ? "当前还无法打开 Stripe Customer Portal。请先完成订阅初始化，或稍后重试。"
            : portalState === "misconfigured"
              ? "账单入口尚未配置完成，请联系管理员检查 Stripe 环境变量。"
              : portalState === "forbidden"
                ? "只有 workspace owner 可以管理账单。"
                : "账单入口暂时不可用，请稍后重试。"}
        </div>
      ) : null}
