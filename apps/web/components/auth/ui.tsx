...
export const inputClass =
  "w-full rounded border border-ink-200 bg-ink-0 px-4 py-2.5 font-ui text-body text-ink-800 placeholder:text-ink-400 transition-all focus:border-ink-900 focus:outline-none disabled:opacity-50";

export const primaryBtnClass =
  "inline-flex h-11 w-full items-center justify-center rounded bg-mauve-200 px-4 font-ui text-body-sm font-medium text-ink-900 shadow-xs transition-colors hover:bg-mauve-100 disabled:opacity-50";
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-mauve-50 px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-lg bg-ink-0 p-8 shadow-lg">{children}</div>
      </div>
    </main>
  );
}
...
export function AuthHeading({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="mb-6">
      <h1 className="font-display text-display-lg font-semibold text-ink-900">{title}</h1>
      {subtitle ? (
