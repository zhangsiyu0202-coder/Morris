...
export interface BuildSystemPromptArgs {
  ...
  /** Session-local derived memory hints (rendered as <derived_memory>). */
  derivedMemories?: ReadonlyArray<MemoryItem>;
  /**
...
export function buildSystemPrompt(args: BuildSystemPromptArgs = {}): string {
  ...
  const derivedMemories = args.derivedMemories ?? [];
  if (derivedMemories.length > 0) {
    sections.push(tag("derived_memory", renderMemories(derivedMemories)));
  }
