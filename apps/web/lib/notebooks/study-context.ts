...
export async function listStudyOptions(
  ...
  const enriched = await Promise.all(
    surveys.map(async (s) => ({
      ...
      title: s.title,
      status: s.status,
      responses: await countCompletedSessions(s.$id),
    })),
  );
  return enriched;
}
...
export async function isValidStudyId(_ownerUserId: string, studyId: string): Promise<boolean> {
...
export async function buildStudyContext(
  ...
  const snippets = await searchTranscriptSegments({
    query: "",
    surveyId: studyId,
    limit: 30,
  });
