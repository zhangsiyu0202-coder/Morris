      ...
      if (baseMetadata.sessionId && baseMetadata.surveyId) {
        ...
        const draft = buildSurveyDraftFromDocs({
          survey: {
            ...
            // T16/T17 (survey-editor): researcher-authored persona on `surveys`.
            // Appwrite returns `null` (not omission) for unset TEXT columns;
            // the mapper coerces null→"" so the contract composer routes to
            // "operational base only".
          ...
          sections: sectionsRes.documents.map((s: any) => ({
            ...
            questions: questionsRes.documents.map((q: any) => ({
              ...
              stimulus: parseJson<{ id: string; type: string; url?: string; text?: string; durationMs?: number } | undefined>(
                q.stimulus,
                undefined,
              ),
            })),
