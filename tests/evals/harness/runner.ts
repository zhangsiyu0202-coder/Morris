...
export async function runEval(opts: RunEvalOptions): Promise<EvalReport> {
  ...
  for (const raw of opts.rows) {
    ...
    const scenarioCap = row.maxTokensOverride ?? perCap;
    if (scenarioTotal > scenarioCap) {
      entries.push({
        id: row.id,
        surface: row.surface,
        tag: row.tag,
        ok: false,
        reason: `cost-exceeded: scenario used ${scenarioTotal} tokens (cap ${scenarioCap})`,
        ...(scoring.judgeFlake ? { judgeFlake: true } : {}),
...
export async function runEvalsCli(): Promise<number> {
  ...
  const { manageMemoriesSurface } = await import("../surfaces/manageMemories");
  const { analyzeDataSurface } = await import("../surfaces/analyzeData");
  const { analyzeSessionTextPassSurface } = await import("../surfaces/analyzeSessionTextPass");
  const { analyzeSurveyCombineSurface } = await import("../surfaces/analyzeSurveyCombine");
  const { combinedEvalScorer } = await import("../scorers/combined");
  ...
  const surfaces = new Map<string, SurfaceAdapter<unknown, unknown>>([
    ...
    [manageMemoriesSurface.name, manageMemoriesSurface as SurfaceAdapter<unknown, unknown>],
    [analyzeDataSurface.name, analyzeDataSurface as SurfaceAdapter<unknown, unknown>],
    [analyzeSessionTextPassSurface.name, analyzeSessionTextPassSurface as SurfaceAdapter<unknown, unknown>],
    [analyzeSurveyCombineSurface.name, analyzeSurveyCombineSurface as SurfaceAdapter<unknown, unknown>],
  ]);
  ...
  const report = await runEval({
    surfaces,
    scorer: combinedEvalScorer as Scorer<unknown, unknown, Record<string, unknown>>,
    rows,
  });
