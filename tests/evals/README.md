...
> Status: complete (all planned surfaces, scorer stack, and nightly workflow landed).
...
│   ├── manageMemories.ts
│   ├── analyzeData.ts
│   ├── analyzeSessionTextPass.ts
│   └── analyzeSurveyCombine.ts
...
`/.github/workflows/evals.yml` runs this harness nightly at 02:00 UTC,
on manual dispatch, and on pushes to `main` that touch the eval corpus or
the covered LLM surfaces. It uploads the latest JSON report as an artifact
and best-effort comments the newest open PR with a one-line summary.
