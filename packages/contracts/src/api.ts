...
export const FinalizeInterviewSessionStateSchema = z.enum(["completed", "abandoned"]);

export const FinalizeInterviewSessionRequestSchema = z.object({
  sessionId: z.string().min(1),
  surveyId: z.string().min(1),
  state: FinalizeInterviewSessionStateSchema,
  ...
  recording: z
    .object({
      ...
      format: z.enum(["mp3", "opus", "wav", "mp4", "webm"]),
    })
    .optional(),
  durationMs: z.number().int().nonnegative(),
