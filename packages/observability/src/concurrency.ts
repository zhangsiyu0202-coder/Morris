 ...
 *
 * NOTE: this default value MUST stay synchronized with
 * `RATE_LIMITS.llmMaxConcurrentDefault` in `@merism/contracts/src/limits.ts`.
 * Observability MUST NOT import from contracts (per
 * `.dependency-cruiser.cjs::observability-no-contracts-import` —
 * observability is more foundational than contracts in the module map).
 * The two values are connected by convention, not import; a property test
 * in `tests/properties/limits-mirror.test.ts` asserts they match.
 */
