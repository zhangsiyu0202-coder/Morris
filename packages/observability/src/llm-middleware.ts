...
import { getRuntimeTraceId } from "./runtime-scope.js";
...
export interface LLMObsMiddlewareOpts {
  ...
  scope: string;
  /**
   * Optional traceId supplier.
   * If omitted, the current runtime traceId is used; if no runtime scope exists,
   * a fresh UUID is generated per call.
   */
...
function resolveTraceId(t: LLMObsMiddlewareOpts["traceId"]): string {
  ...
  return getRuntimeTraceId() ?? randomUUID();
}
