...
| Usage metering (billable unit = a completed interview session) + quotas | `apps/functions/aggregateWorkspaceUsage/`, `apps/functions/issueLivekitToken/` (entry-gate), `apps/functions/finalizeInterviewSession/` (UsageEvent emit on `terminalStatus=completed`) |
