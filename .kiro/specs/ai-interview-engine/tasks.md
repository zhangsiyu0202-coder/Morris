...
- [x] **G3** 确认主持指令通路:`InterviewSupervisorAgent(instructions=workflowConfig.supervisorInstruction)`,且 `buildInterviewWorkflowConfigFromDraft` 已把 `Survey.moderatorInstruction` 合成进该串。**survey-editor T16 (契约合成层) 已落地;Function 侧 mapper / deps.ts 透传缺口已补 (`apps/functions/issueLivekitToken/src/{survey-draft-mapper.ts,deps.ts}`, mapper.test 加 passthrough 守护),整条链接通**。agent 侧无需改:Python `workflow.py::workflow_config_from_metadata` 一直优先消费 TS 端合成好的 `workflowConfig`,`_supervisor_instruction_from_study` 仅作 fallback。
