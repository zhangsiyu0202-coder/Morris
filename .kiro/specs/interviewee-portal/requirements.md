...
- **mock 残留已清(2026-06-30)**:`lib/mock-session.ts` + `lib/hooks/use-interview-session.ts` 已移除;`app/page.tsx` 改为 server redirect → `/home`(`middleware.ts` 同步把 `/` 加进 matcher)。根预览与 live `useLiveInterview` 的双轨已消除,不再是收口债。
