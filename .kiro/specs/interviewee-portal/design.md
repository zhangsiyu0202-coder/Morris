...
- **D3 mock 残留已清(2026-06-30)**:`lib/mock-session.ts` + `lib/hooks/use-interview-session.ts` 已移除,`app/page.tsx` 改为 server redirect → `/home`(`middleware.ts` 同步把 `/` 加进 matcher),受访者端不再有"mock 预览 vs live"双轨。
