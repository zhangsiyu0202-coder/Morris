...
- [ ] **D1**(live e2e)受访者端真 stack 端到端:`/interview?link=<token>` → 预访谈(设备/屏幕共享/同意)→ 加入 → 转写流 → 提交答案 RPC → 完成。gated `MERISM_LIVE_TESTS=1`,需 `pnpm stack:up` + agent(`uv sync --extra realtime`)+ fake providers。**本轮 stack/agent 不可用即记 NEXT。**
