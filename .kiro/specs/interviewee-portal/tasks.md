...
- [x] **D2**(完成)`lib/mock-session.ts` + `lib/hooks/use-interview-session.ts` 已移除;`app/page.tsx` 改为 server redirect 到 `/home`,`middleware.ts` 同步把 `/` 加入 matcher。受访者端不再有"mock 预览 vs live"双轨。
