...
- [x] 9. agent worker 链式触发
  - 在 `apps/functions/finalizeInterviewSession/` 新建 pure-core + SDK-wrapper Function，负责 finalized transcript/session/usage/report dispatch；recording 文件仍由 agent 直传 storage，metadata 经该 Function upsert。
  - 在 supervisor wrap-up 链路尾部调用 `finalizeInterviewSession`；Function 内部再链式触发 `analyzeSession` / `analyzeSurvey`。任一失败用 `agent.logging.create_logger(scope)` 记录 traceId，但不影响 session completed 状态的持久化。
  - Python 单测用 fake Functions client 验证：wrap-up 最终只 dispatch 一次 `finalizeInterviewSession`，且 payload 携带 final transcript / answers / optional recording metadata。
