...
agent 的 wrap-up 阶段现在通过 `finalizeInterviewSession` Function 收口最终 transcript / session / usage event / analyzeSession+analyzeSurvey 链路；recording 文件仍由 agent 直传 storage，metadata 通过同一个 finalize Function 写入 `recordings` document。

```python
async def finalize_session(...):
    await self.functions.create_execution(
...
调用是 fire-and-await（等 Function 完成）但失败不回滚 recording 文件。失败时 logger 记录 `traceId`，下次研究员进 `/reports/[surveyId]` 仍可手动"重新生成"。

### 9.2 权限

agent server key（`APPWRITE_API_KEY`）的 scope 增加：

- `functions.execute` 至少包含 `finalizeInterviewSession`（其内部再链式触发 `analyzeSession`、`analyzeSurvey`）
- `databases.write` 不再承担 transcript/session/report finalized 主路径；那部分交由 Function server key 写入。agent 保留 recording bucket 上传所需的 storage scope。
