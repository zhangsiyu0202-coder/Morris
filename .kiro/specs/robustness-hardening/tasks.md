...
5. **`no-inline-vi-mock-duplicate` shape** (REQ-1): semgrep still cannot count duplicates across files. The landed rule instead fail-closes on the known repeated Morris test-double module paths (`@/lib/queries`, `@/lib/server/notebooks`, `@/lib/server/embedder-qwen`, `@/lib/actions/survey`, `node-appwrite`) and requires centralized fixtures rather than inline object factories.
6. **EXEMPT_PREFIXES grew** (Task 1.3 + Phase A wiring): added `apps/web/lib/assistant/access-control.ts`, `__tests__/access-control.test.ts`, `app/api/assistant/route.ts`, `lib/assistant/tool-types.ts` to `scripts/scope-guard.ts` exempt list. Same-PR rationale tied to ADR-0006 / REQ-4 per `scope.md`.

### Wave B follow-up sub-specs (to be created)

Listed in priority order:

1. **`ai-eval-suite`** (REQ-7) — pytest + scorer harness for LLM behavior regression. Highest-value for ongoing AI quality.
2. **`resource-limits-registry`** (REQ-8) — typed limit catalog + evaluator, depends on `workspaces-billing` Wave 2.
3. **`dependency-cruiser-boundaries`** (REQ-6) — TS module boundary enforcement. Depends on REQ-1 conventions.
4. **`prompt-versioning`** (REQ-9) — analysis Function prompt directory layout + golden tests. Depends on REQ-7 harness.
