...
1. 契约：编辑态唯一真源 = `@merism/contracts` `SurveyDraft`（最终未保留额外的跳过布尔位）。
...
影响面与处理：
- `buildInterviewRuntimeStudy` / `buildInterviewWorkflowConfigFromDraft`：不包含额外的跳过布尔位，运行时跳题/追问语义仍由 `ai-interview-engine` 自主决定；保持 `pnpm typecheck` 绿。
- `apps/agent/agent/contracts.py`：draft question 镜像与 TS 契约一致，不镜像该布尔位。
...
- `QuestionBlock.type → questionType`，`probeConfig.level/instruction → probeLevel/probeInstruction`，`config.options → options`，`stimulus` 直传。
...
`@merism/appwrite-schema`：确认 `surveys` / `survey_sections` / `question_blocks` 的 attributes 覆盖 §4 映射所需字段（`question_blocks.config` 容纳 `options`；`probeConfig` 已在契约）。索引：`survey_sections.surveyId`、`question_blocks.surveyId`（read layer 已按其查询）。权限：三表写权限仅授予研究员（owner），匿名角色无任何写、无跨用户读。`apply` 保持幂等非破坏。
