...
2. 编辑态契约从本地 `apps/web/lib/guide.ts` 收敛到 `@merism/contracts` 的 `SurveyDraftSchema`（不新增额外的跳过布尔位）。
...
**包含**：
- 契约收敛：`SurveyDraftQuestionSchema` 以 `@merism/contracts` 为准，最终不新增额外的跳过布尔位；删除/退役 `apps/web/lib/guide.ts` 中与契约重复的类型，编辑器改用 `@merism/contracts`
...
- 跳过逻辑/skip-logic 的运行时语义（不通过额外的编辑态布尔位承载；agent 侧自行决定）
...
#### Acceptance Criteria
1. WHEN 需要表达"允许跳过" THEN 编辑器 SHALL 不通过 `SurveyDraftQuestionSchema` 新增额外布尔位来承载；如需支持，必须先走新的架构决策，再改 `packages/contracts`。
...
- **P-DATA-01**：对任意合法 `SurveyDraft`，`saveSurveyDraft` 后再 `getStudy` 组装出的 draft 与原 draft 语义等价（分节/问题顺序、题型、选项、probe 全部保持）。
