...
- [x] **T1** 从 `packages/contracts/src/api.ts` 回收编辑态的跳过布尔位；导出类型不变。跑 `pnpm -F @merism/contracts typecheck` 与契约测试。 — 已落地（已删除）。
- [x] **T2** 修复该字段删除引入的全仓破坏点：`buildInterviewRuntimeStudy`/`buildInterviewWorkflowConfigFromDraft` 不再消费；同步清理 `apps/agent/agent/contracts.py` 中对应镜像（若存在）。`pnpm typecheck` 全绿。 — 已确认 `pnpm typecheck` 全绿。
...
- [x] **T4** 新增纯函数 `apps/web/lib/survey-draft.ts::assembleSurveyDraft(survey, sections, questions): SurveyDraft`（§4 映射，含 `flowConfig` meta、`config.options`、status 映射 D5）。加单测。 — 已落地；P-DATA-01 往返测试通过。
...
- [x] **T9** `@merism/appwrite-schema`：确认/补 `survey_sections`/`question_blocks` 的 attributes（`config` 容纳 options）、索引（`surveyId`）、权限（写仅 owner 研究员，匿名无写）。`pnpm schema:apply` 幂等非破坏，`pnpm schema:verify` 通过。 — 已落地；`pnpm schema:verify` 对 live stack 通过（2026-06-11）。
