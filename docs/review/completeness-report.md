...
| Wave 8 | **T16. 本地开发栈冒烟测试脚本** | 已完成 | `scripts/smoke.sh`, `scripts/smoke.mts`, root `pnpm smoke` verified (`issueLivekitToken` + `finalizeInterviewSession`). |
...
| **6. Analysis report generation** | **EDGE** | `finalizeInterviewSession` now triggers `analyzeSession` / `analyzeSurvey` through deployed Functions, and local smoke covers that boundary. Remaining risk is downstream provider/runtime variance, not missing trigger wiring. | Add an end-to-end report viewer assertion on top of the existing smoke if stricter release confidence is needed. |
