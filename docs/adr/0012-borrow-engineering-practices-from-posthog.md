...
- **2026-07-02** — Added the fifth Wave A rule, `no-inline-vi-mock-duplicate`, in a semgrep-compatible first-cut form: fail-closed for the known repeated Morris test-double module paths that must use centralized fixtures instead of inline object factories.
