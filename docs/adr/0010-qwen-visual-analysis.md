...
- The `openruntimes/python:v4-3.12` image is already present in the local Docker stack (`OPR_EXECUTOR_RUNTIMES` includes `node-21.0,node-22.0,python-3.12` in `docker-compose.yml`) and Appwrite 1.6 natively supports Python Functions.
