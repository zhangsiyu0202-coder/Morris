...
Walks through deploying the core Functions in `apps/functions/*` against
...
```bash
# 1. all core Functions built (status=ready, buildSize > 1 MB confirms node_modules included)
. .env
for FN in issueLivekitToken finalizeInterviewSession analyzeSession analyzeSurvey analyzeSessionVisual; do
...
- **30 s sync timeout is hardcoded in Appwrite 1.6**. Functions that
  ...
  to Gemini Vision) must run with `async=true`. The agent worker now
  dispatches `finalizeInterviewSession` sync (it persists transcript/session,
  emits usage, then triggers analyzeSession + analyzeSurvey inside the Function);
  analyzeSessionVisual is still dispatched async by the egress finalize hook.
...
- **Scheduling**. Functions can be scheduled (`schedule` field on the
  ...
  call from the agent worker (`finalizeInterviewSession` /
  `enqueue_visual_analysis`) or the browser bootstrap (`issueLivekitToken`).
