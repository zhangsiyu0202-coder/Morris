...
- **Agent** — a stdlib HTTP server on `HEALTH_PORT` (default `8082`) inside
...
curl http://localhost:8082/_livez          # agent
```
...
curl http://localhost:8082/_readyz                    # agent
```
...
curl -i http://localhost:8082/_livez
curl -i http://localhost:8082/_readyz
```
...
|---|---|---|---|
| `HEALTH_PORT` | int | `8082` | Agent health server listen port |
